...
<text>
[{"t":"<#=@tjy#>统计月13个月年累计继续率报表-个险","stl":{"sz":28}}]
</text>
</WidgetText>
</body>
</WidgetSheet>
...
select *
  from (select 机构.分公司 as fgs,
               ...
               sum(C_13个月.保单件数分母_13_Y) as bdjsfm,
               sum(C_13个月.保单件数分子_13_Y) as bdjsfz,
               sum(C_13个月.AP分母_13_Y) as apfm,
               sum(C_13个月.AP分子_13_Y) as apfz,
               max(统计区间.START_13_YEAR) sxrq,
               max(统计区间.END_13_YEAR) sxrz
          from DMART.CDM_DIM_CONTINUITY_MANAGECOM 机构,
               ...
               (select A.STARTDATE_ID        STATDATE,
                       ...
                       A.APBELOW_Y           AP分母_13_Y,
                       A.APUP_Y              AP分子_13_Y,
                       A.CONTCALCOUNTBELOW_Y 保单件数分母_13_Y,
                       A.CONTCALCOUNTUP_Y    保单件数分子_13_Y
